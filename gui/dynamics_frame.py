...
class DynamicsFrame(QWidget):
    ...
    def display_newton_results(self):
        ...
        html_parts = []
        html_parts.append("<b style='font-size:13px;'>RESULTADOS DEL CÁLCULO (2ª Ley de Newton)</b><br>")
        html_parts.append("=" * 50 + "<br><br>")

        html_parts.append("<b>Parámetros utilizados:</b><br>")
        html_parts.append("-" * 25 + "<br>")
        param_map = {
            'f': 'Fuerza Aplicada (N)', 
            'm': 'Masa (kg)', 
            'a': 'Aceleración (m/s²)',
        ...
        html_parts.append("<br><b>Resultado calculado:</b><br>")
        html_parts.append("-" * 25 + "<br>")
        ...
        if normal_force is not None:
            html_parts.append("<br><b>Fuerza Normal:</b><br>")
            html_parts.append("-" * 25 + "<br>")
            html_parts.append(f"&nbsp;&nbsp;• F_normal: {normal_force:.4f} N<br>")

        friction_force = self.results.get('friction_force')
        if friction_force is not None:
            html_parts.append("<br><b>Fuerza de Fricción:</b><br>")
            html_parts.append("-" * 25 + "<br>")
            html_parts.append(f"&nbsp;&nbsp;• F_fricción: {friction_force:.4f} N<br>")

        html_parts.append("<br><b style='color:#9b59b6;'>Ecuaciones utilizadas:</b><br>")
        html_parts.append("-" * 25 + "<br>")
    ...
    def display_energy_result(self, title, value, equation, params):
        html_parts = []
        html_parts.append(f"<b style='font-size:13px;'>RESULTADO - {title.upper()}</b><br>")
        html_parts.append("=" * 50 + "<br><br>")

        html_parts.append("<b>Parámetros:</b><br>")
        for name, val in params.items():
            if val is not None:
                html_parts.append(f"&nbsp;&nbsp;• {name}: {val}<br>")
        
        html_parts.append(f"<br><b>Ecuación:</b><br>&nbsp;&nbsp;• {equation}<br>")
        html_parts.append(f"<br><b>Resultado:</b><br>&nbsp;&nbsp;• <span style='color:#2ecc71;'>{title}</span>: {value:.4f} Joules<br>")
